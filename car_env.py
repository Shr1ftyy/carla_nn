...
class CarEnv:
    def __init__(self, port=2000): 
        ...
        self.sensor_list = {} # --> Sensor List (dict)
        ...
        #SENSOR INIT
        self.semantic = self.blueprint_library.find('sensor.camera.semantic_segmentation')
        self.semantic.set_attribute("sensor_tick", f"{TICKRATE}")
        self.semantic.set_attribute("image_size_x", f"{self.im_width}")
        self.semantic.set_attribute("image_size_y", f"{self.im_height}")
        self.semantic.set_attribute("fov", f"110")
        self.semantic.set_attribute("role_name", f"semantic")
        ...
        self.sensor_list['front'] = (self.frontCam)
        self.semanticCam = self.world.spawn_actor(self.semantic, self.frontTrans, attach_to=self.vehicle_list[0])
        self.sensor_list['semantic'] = (self.semanticCam)
        # self.leftCam = self.world.spawn_actor(self.rgb_cam, self.leftTrans, attach_to=self.vehicle_list[0])
        # self.sensor_list.append(self.leftCam) # self.rightCam = self.world.spawn_actor(self.rgb_cam, self.rightTrans, attach_to=self.vehicle_list[0])
